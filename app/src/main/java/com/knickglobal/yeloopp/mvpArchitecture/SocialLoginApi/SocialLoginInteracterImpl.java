...
public class SocialLoginInteracterImpl implements SocialLoginInteracter {

    private static final String TAG = "Beant LoginIntrImpl";

    @Override
    public void onSocialLogin(Activity activity, HashMap<String, String> map,
                              final SocialLoginPresenter.OnSuccess onSuccess) {

        ApiInterface apiInterface = ApiClient.getClient().create(ApiInterface.class);

        Call<LoginPojo> call = apiInterface.socialLoginApi(map);
            ...
            public void onResponse(@NonNull Call<LoginPojo> call, @NonNull Response<LoginPojo> response) {
                if (response.body() != null) {
                    if (response.body().getStatus().equalsIgnoreCase("1")) {
                        onSuccess.onSocialLoginSuccess(response.body());
                    } else {
                        onSuccess.onFailSocialLogin(response.body().getMessage());
                    }
                } else {
                    onSuccess.onFailSocialLogin(AppConstants.SOMETHING_WENT_WRONG);
                }
            }
